...
# ============================== Service Worker للجذر (PWA + OneSignal)
@app.get("/sw.js")
def sw_js():
    path = os.path.join(STATIC_DIR, "pwa", "sw.js")
    return FileResponse(path, media_type="application/javascript")

# OneSignal workers at root
...
def admin_export(request: Request):
    ...
    with db() as con:
        ...
        for row in cur:
            writer.writerow([row["id"], row["ts"], row["type"], row["query"] or "",
...
def send_push(title: str, body: str, url_path: str = "/") -> bool:
    """إرسال إشعار OneSignal لجميع المشتركين — (Bearer v16)"""
    if not (ONESIGNAL_APP_ID and ONESIGNAL_REST_API_KEY and PUBLIC_BASE_URL):
        return False
    full_url = url_path if (url_path or "").startswith("http") else (PUBLIC_BASE_URL.rstrip("/") + (url_path or "/"))
    payload = {
    ...
    try:
        ...
        try: log_event("push", "server", "scheduler", query=title, engine_used=f"os:{'ok' if ok else 'fail'}")
        except: pass
        return ok
    except Exception:
        return False
...
def start_scheduler():
    ...
    # إشعار يومي ثابت الساعة 3:00 عصرًا (بتوقيت مكة)
    sch.add_job(job_daily_digest_15, CronTrigger(hour=15, minute=0, timezone=TIMEZONE))
    # إشعار قبل 30 دقيقة ووقت البداية — فحص كل 5 دقائق
