# main.py — Bassam AI (بحث نصّي + بحث بالصور + PWA جاهز)
...
async def smart_search(q: str, prefer_google: bool = True, num: int = 8) -> Dict:
    """يجرّب Google (إن توفر المفتاح) وإلا DuckDuckGo."""
    try:
        results, used = [], None
...
async def upload_image(request: Request, file: UploadFile = File(...)):
    ...
    try:
        ...
        # اسم ملف آمن وفريد
        base_ext = (file.filename.split(".")[-1] or "jpg").lower()
        if base_ext not in ["jpg", "jpeg", "png", "webp", "gif"]:
            base_ext = "jpg"
        filename = f"{uuid.uuid4().hex}.{base_ext}"

        save_path = os.path.join(UPLOADS_DIR, filename)
        with open(save_path, "wb") as f:
            f.write(await file.read())

        # محاولة تحديد الرابط العام تلقائيًا لو لم يتم ضبط PUBLIC_BASE_URL
