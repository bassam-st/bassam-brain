...
# ========= بحث الويب (الملف الخارجي) =========
# core/search_web.py يحتوي web_search + summarize_snippets
try:
    from core.search_web import web_search, summarize_snippets
except Exception:
    # نسخة بديلة بسيطة في حال لم يتوفر الملف لأي سبب
...
async def smart_answer(question: str) -> Tuple[str, dict]:
    ...
    # 2) احتياطي ويب
    results = await web_search(q, max_results=6) if callable(getattr(web_search, "__call__", None)) else []
    if results:
